...
> 현재 상태(7c-1): 로그인·인증 저장·보호 라우트·API 클라이언트까지. **레이아웃(사이드바 + 상단 바)과 디자인 토큰은 승인 대기(7c-1b)**라 메뉴는 스타일 없는 임시 링크이고, 팀·곡·유튜브 화면은 "준비 중" 자리표시 페이지다(7c-2~4).

## 스택

Next.js 16.2.4(App Router) · React 19.2.4 · TypeScript · Tailwind 4 · shadcn/ui(radix-nova; 색·radius·폰트 토큰은 임시, 디자인 승인 대기) ·
...
src/
  ...
  components/         AuthGate(보호 라우트 + 임시 메뉴), LoginForm, SessionExpiredDialog, ui/(shadcn)
  lib/
