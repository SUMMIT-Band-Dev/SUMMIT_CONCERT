...
- **커밋**: 마이그레이션 `49ca245` / 클라이언트·스코어링 `15b7f13` / 배치 `201ba44` / 리뷰 `771d0a0` / F013·songs 연동 `389afa5` / 중단 응답 혼합안 `bf642c9` / PRD 정정 `75b9bc5` / 이 문서
...
| 썸네일 호스트 | 공식 문서의 `search` 리소스 스키마에 `snippet.thumbnails.*.url`은 있지만 **예시 URL의 호스트가 없다** | **실측 확정(게이트 2): 응답 150건 전부 `i.ytimg.com`**(`default.jpg`·`mqdefault.jpg`·`hqdefault.jpg` 3종, 형태 `https://i.ytimg.com/vi/<id>/<size>.jpg`). allowlist의 `img.youtube.com`은 **한 번도 관측되지 않은 추측값**으로 남아 있다(제거를 권장 — 이월) |
...
- **후보 3개를 고르는 기준 — 이식한 점수가 원본 순서보다 나쁘다 (결정 필요, 게이트 2에서 발견).** 승인된 5곡에서 정답이 YouTube 원본 순서로는 상위 4위 안(상위 3위 안 4/5)인데, 점수로 재정렬하면 상위 3에 드는 것이 1/5, 1위가 정답인 곡은 0/5였다. 임시 곡 A(`혜성 / 윤하`)는 후보 3개가 **전부 77점 동점**이라 결국 원본 순서로 정해졌다 — 점수가 변별력을 못 갖는 경우가 흔하다는 뜻이다. **같은 점수로 1위를 고르는 프론트 실시간 폴백이 지금 방문자에게 여는 영상도, 이 5곡 기준으로 사람이 승인한 영상과 5/5 다르다**(다른 것이 곧 틀린 것은 아닐 수 있다). 선택지: (a) 원본 순서 상위 3개를 그대로 저장, (b) 점수는 동점·감점(라이브·커버) 필터로만 쓰고 순서는 원본 유지, (c) 현행 유지. 표본이 5곡이므로 결정 전에 더 많은 곡(예: 59곡 중 일부)으로 확인하는 편이 낫다. 어느 쪽이든 프론트 폴백과의 로직 두 벌 문제(아래)와 함께 정리한다
...
- **썸네일 호스트 — `i.ytimg.com`으로 확정됐다(게이트 2).** 7단계에서 `next.config.ts`의 `images.remotePatterns`에 `i.ytimg.com`을 추가하거나 `<img>`를 쓴다. 저장 allowlist(`YOUTUBE_THUMBNAIL_ALLOWED_HOSTS`)에는 **관측되지 않은 `img.youtube.com`이 아직 들어 있다** — 관측한 것만 허용하는 쪽이 안전하므로 제거를 권장한다
