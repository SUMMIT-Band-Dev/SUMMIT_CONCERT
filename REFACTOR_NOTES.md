...
- **썸네일 표시용 이미지 도메인** — `next.config.ts`의 `images.remotePatterns`는 `**.supabase.co`와 `is1-ssl.mzstatic.com` **둘뿐**이라 YouTube 썸네일은 `next/image`로 띄울 수 없다. 서버의 저장 allowlist(`youtube-search.constants.ts`의 `YOUTUBE_THUMBNAIL_ALLOWED_HOSTS`, 현재 `i.ytimg.com`·`img.youtube.com`)는 **추측이며 게이트 2 캘리브레이션에서 실측해 확정**한다. 확정되면 두 곳을 **같은 값으로 함께** 고친다(§13의 앨범 커버 allowlist와 같은 방침). 또는 `<img>`를 쓰면 `remotePatterns`를 건드리지 않아도 된다
...
- **배치 중단 응답 방식 결정** — 지금은 `200` + `abortedBy`다(§15 "남겨둔 결정"). 클라이언트가 상태 코드로 실패를 판단할 수 있게 `429`/`500`으로 바꿀지 결정하고, 그에 따라 쓰이지 않는 메시지 상수 4개를 살리거나 지운다
...
- **`prd-admin.md:202` 정정** — "배치용 API 키와 폴백용 API 키를 분리해 방문자 quota를 보호"는 **키를 나눠서는 성립하지 않는다**(쿼터는 Cloud 프로젝트 단위, §15 조사). 프로젝트를 나눈다는 표현으로 고친다
...
- **커밋**: 마이그레이션 `49ca245` / 클라이언트·스코어링 `15b7f13` / 배치 `201ba44` / 리뷰 `771d0a0` / F013·songs 연동 `389afa5` / 이 문서
- **관련 PR**: work02-6b, `feat: 유튜브 배치 추천 검색 및 추천 리뷰(승인/반려) API 구현` (푸시/PR 생성은 보류)
- **상태**: 구현·단위 테스트·프로덕션 SELECT 전용 검증까지 완료. **실제 YouTube API 호출(게이트 2), 임시 데이터로 쓰기 검증, 59곡 전체 배치는 아직 하지 않았다** — "검증"의 "아직 하지 않은 것" 참조
...
| 썸네일 호스트 | 공식 문서의 `search` 리소스 스키마에 `snippet.thumbnails.*.url`은 있지만 **예시 URL의 호스트가 없다** | allowlist(`i.ytimg.com`, `img.youtube.com`)는 **추측**이며 게이트 2 캘리브레이션에서 실측해 확정한다 |
...
| `POST` | `/youtube/recommendations/batch` | `{ limit? }`(기본 5, 최대 10). 대상을 서버가 선정해 청크 처리. 요약(처리/검색됨/결과 0건/실패/중단 사유/남은 대상/쿼터) 반환 |
...
| 테이블 구조 | **시도 + 후보 2개**, 후보 상위 3개 | 곡당 후보 N행 단일 테이블로는 "검색했는데 0건"을 표현할 행이 없다 |
...
**단위 테스트** — 전체 **690개 / 39파일**(1/2 종료 시 462개 / 27파일). `apps/api`의 `build`·`lint`(경고 0)·`test` 통과. 스펙 포함 타입 검사(`tsc --noEmit -p tsconfig.json`)는 **이번 브랜치가 만든 오류 0건**이고 기존 8건(§10)만 남았다. 고정한 것: 스코어링 특성 테스트(프론트 구현 복사본과 값 비교), 클라이언트의 실패 분류와 **키가 에러·스택·cause·직렬화 어디에도 없음**, `readBatchApiKey`의 기동 검증(0/19/20자 경계, 프론트 키 이름을 읽지 않음), 배치의 예약 순서·실패 분류표 각 칸·중단 조건·부분 성공·중복 실행 409, 승인 경합 409·재승인 409·후보 밖 영상 400·DB 예약어 방어, 재큐 대상 조건, 30일 경계(29일 보임 / 31일 제외), 잠금 순서, 컨트롤러 `@Public()` 미부착과 라우트 선언.
...
**아직 하지 않은 것** (별도 승인 사항)
- **실제 YouTube API 호출.** `X-goog-api-key` 헤더를 실제 API가 받아들이는지, 응답의 썸네일 호스트, `quotaExceeded`의 reason 문자열을 실제로 받아 보지 못했다 — 모두 문서에 기댄 가정이다
- **쓰기 경로의 실DB 실행.** 예약·후보 저장·승인·반려·재큐·F013/songs 연동이 CHECK 제약과 FK 아래서 실제로 통과하는지는 코드로만 대조했고 실행하지 않았다(`outcome`×`reviewState` 조합은 코드가 만드는 값이 전이표와 일치함을 읽어서 확인). 임시 곡(day98)으로 검증한다
- 동시 승인 2건 → 성공 1 + 409 1, F013 수동 입력과의 경합의 실DB 재현
- 59곡 전체 배치
...
- **배치 중단 시 응답 방식 — 설계 초안과 다르게 구현했다.** 초안은 쿼터 초과를 `429 + Retry-After`, 키 오류를 `500`으로 매핑하는 것이었는데, 구현은 **`200` + 요약의 `abortedBy`(`quota`/`api_key`/`consecutive_errors`)** 로 돌려준다. 이미 저장한 부분 성공을 응답에 그대로 담을 수 있는 장점이 있지만 클라이언트가 상태 코드만으로 실패를 알 수 없다. 결정 전까지 `youtube-search.constants.ts`의 메시지 상수 4개(`QUOTA_EXHAUSTED`/`UPSTREAM`/`TIMEOUT`/`API_KEY`)는 **어디서도 쓰이지 않는 채 남아 있다** — 429/500 매핑을 택하면 이 상수가 쓰이고, 200을 확정하면 지운다
...
- **썸네일 호스트 allowlist 확정**, 그리고 확정되면 `next.config.ts`의 `images.remotePatterns`와 함께 고친다(§13의 앨범 커버 allowlist와 같은 방침)
...
- **스코어링 로직이 프론트와 두 벌이다** — URL이 채워질수록 프론트 폴백 호출이 줄어드므로 7단계 이후 폴백 라우트 제거를 검토한다. 그때까지 프론트 구현이 바뀌면 특성 테스트의 복사본도 함께 갱신해야 한다
